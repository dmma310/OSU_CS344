...
int main(int argc, char* argv[]) {
    ...
    int ok_len = strlen(SVR_ALLOW_MSG), cli_id_len = strlen(DEC_CLI_CONFIRM_MSG);
    ...
    while (1) {
        ...
        case 0:
        {
            ...
            char full_decrypted_msg[FULL_MSG_SIZE];
            ...
            if (strcmp(buffer, DEC_CLI_CONFIRM_MSG) != 0) {
            ...
            strcpy(cipher_text, token);
            token = strtok_r(NULL, FILE_DELIMITER, &rest);
            strcpy(key_text, token);
            // Produce and get decrypted message
            decrypt(full_decrypted_msg, cipher_text, key_text);
            int full_decrypted_msg_len = strlen(full_decrypted_msg);

            /*****************Send decrypted message to client*****************/
            int decrypt_msg_len = strlen(full_decrypted_msg);
            if ((sendall(connected_client_fd, full_decrypted_msg, &decrypt_msg_len)) == -1) {
                fprintf(stderr, "SERVER: ERROR cannot write to client\n");
            }
            exit(0);
        }
